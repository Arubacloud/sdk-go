...
**Aggiornamento di un Cloud Server**

`Update` è un dispatcher intelligente: ispeziona lo stato in sospeso del wrapper e chiama gli
endpoint API corretti in sequenza. Si chiama `Update` una sola volta indipendentemente da cosa è cambiato.

```go
// Solo rinomina → PUT /cloudServers/:id
cs.Named("nuovo-nome")
cs, err = arubaClient.FromCompute().CloudServers().Update(ctx, cs)

// Solo subnet → POST …/associateDisassociateSubnets
cs.AssociateSubnets(nuovaSubnet).DisassociateSubnets(vecchiaSubnet)
cs, err = arubaClient.FromCompute().CloudServers().Update(ctx, cs)

// Modifiche multiple in una sola chiamata → PUT + subnet + volume in sequenza
cs.Named("nuovo-nome").
    AssociateSubnets(nuovaSubnet).
    AssociateSecurityGroups(nuovoSG).
    DisassociateSecurityGroups(vecchioSG).
    AttachDataVolumes(nuovoVol).
    DetachDataVolumes(vecchioVol)
cs, err = arubaClient.FromCompute().CloudServers().Update(ctx, cs)
```

Se non è cambiato nulla (nome/tag invariati, nessuna coda delta impostata), `Update` è un no-op (zero chiamate API).
Le sotto-chiamate sono sequenziali; in caso di errore il wrapper riflette lo stato dopo l'ultima chiamata riuscita.
...
**Setter delta per Update** (code le modifiche inviate da `Update`):
- *Subnet*: `AssociateSubnets(...Ref)`, `DisassociateSubnets(...Ref)`
- *Security group*: `AssociateSecurityGroups(...Ref)`, `DisassociateSecurityGroups(...Ref)`
- *Elastic IP*: `AssociateElasticIPs(...Ref)`, `DisassociateElasticIPs(...Ref)`
- *Data volume*: `AttachDataVolumes(...Ref)`, `DetachDataVolumes(...Ref)`
